...
def plot_pcs(
    ...
    for pc_idx in pcs_to_plot:
        image_path = os.path.join(
            plot_dir, f"brain_pc_{pc_idx + 1}_{metric_to_plot}_fc.png"
        )
        json_path = os.path.join(
            metadata_dir,
            f"brain_pc_{pc_idx + 1}_{metric_to_plot}_metadata.json",
        )
...
def _():
    mo.md(r"""
    ## Extraction and harmonization of the regular FC matrices of the SRPB dataset
    """)
    return
...
def _():
    mo.md(r"""
    Here is a breakdown of the method to estimate this different bias:
    - Subject
    - Site
    - Sampling
    - (Protocol)
    ...
    1. Extract the TS (Traveling Subjects) and RS (Regular Subjects) connectivity matrices and metadata
...
def _():
    mo.md(r"""
    We also want to look at how the Fuzzy `np.corrcoef` impacts the FC matrices themselves
    """)
    return
...
@app.function
def analyze_fuzzy_fc_matrix_impact(
    regular_fc_df: pl.DataFrame,
    fuzzy_fc_df_list: list[pl.DataFrame],
    out_dir: str,
    fc_col: str = "fc_matrix",
) -> mo.Html:
    os.makedirs(out_dir, exist_ok=True)
    stats_path = os.path.join(out_dir, "fuzzy_fc_impact_stats.json")

    if os.path.exists(stats_path):
        print(f"Loading cached fuzzy FC impact analysis from {out_dir}")
...
"""
    ...
    print(f"Computing fuzzy FC impact analysis...")

    print(
        f"Extracting regular FC matrices ({regular_fc_df.height} subjects)..."
    )
    regular_fc = np.stack(regular_fc_df[fc_col].to_numpy())
    n_subjects, n_edges = regular_fc.shape
    n_runs = len(fuzzy_fc_df_list)
    print(f"  Regular FC shape: {regular_fc.shape}")
    ...
    for run_idx, df in enumerate(fuzzy_fc_df_list):
        ...
        if run_max_abs > global_max_abs:
            global_max_abs = run_max_abs
    print(f"  Max |difference|: {global_max_abs:.3e}")

    abs_bin_edges = np.linspace(0, global_max_abs, 101)
    ...
    abs_bin_centers = (abs_bin_edges[:-1] + abs_bin_edges[1:]) / 2
    median_idx = np.searchsorted(cumulative, total_count * 0.5)
    median_abs_diff = float(
        abs_bin_centers[min(median_idx, len(abs_bin_centers) - 1)]
    )
...
def _(
    ...
    srpb_fuzzy_fc_impact_ui = analyze_fuzzy_fc_matrix_impact(
        srpb_extracted_fc_matrices_df,
        srpb_fuzzy_extracted_fc_matrices_df_list,
        out_dir=srpb_fuzzy_fc_matrices_impact_output_dir,
    )

    srpb_fuzzy_fc_impact_ui
    return
...
def _():
    mo.md(r"""
    We also want to run the PCA features extraction on the extracted SRPB FC matrices
    """)
    return


@app.cell
def _(srpb_extracted_harmonized_fc_matrices_df):
    srpb_extracted_harmonized_fc_matrices_hc_mdd_df = (
        srpb_extracted_harmonized_fc_matrices_df.filter(
            pl.col("diag").is_in([0, 2])
        )
    )
    return (srpb_extracted_harmonized_fc_matrices_hc_mdd_df,)
...
def _(
    ...
    srpb_extracted_metrics_dict = calculate_metrics(
        df=srpb_filter(srpb_extracted_harmonized_fc_matrices_hc_mdd_df),
        metric_dict=metric_dict,
        alpha_threshold=0.05,
        n_pcs=5,
        plot_dir=srpb_fuzzy_plot_dir + f"/extracted-fc-matrices/",
        ttest_dir=srpb_fuzzy_ttest_dir + f"/extracted-fc-matrices/",
        cache_dir=srpb_fuzzy_cache_dir + f"/extracted-fc-matrices/",
    )

    srpb_extracted_metrics_results = srpb_extracted_metrics_dict["results"]
    srpb_extracted_metrics_selected_pcs = srpb_extracted_metrics_dict[
        "selected_pcs"
    ]
    srpb_extracted_metrics_ui = srpb_extracted_metrics_dict["ui"]
    return srpb_extracted_metrics_results, srpb_extracted_metrics_ui
...
@app.cell
def _(srpb_extracted_metrics_ui, srpb_fuzzy_metrics_ui_list):
...
def _():
    mo.md(r"""
    We also want to run the FC plots on the extracted SRPB FC matrices
    """)
    return
...
def _(
    ...
    network_assignments,
    srpb_extracted_harmonized_fc_matrices_hc_mdd_df,
    srpb_extracted_metrics_results,
    srpb_fuzzy_metadata_dir,
    srpb_fuzzy_plot_dir,
):
    srpb_extracted_target_pcs_to_plot = [1.0]

    srpb_extracted_pc_plots_results = plot_pcs(
        results=srpb_extracted_metrics_results,
        fc_matrices_df=srpb_extracted_harmonized_fc_matrices_hc_mdd_df,
        node_coords=coords_mni,
        pcs_to_plot=srpb_extracted_target_pcs_to_plot,
        metric_to_plot="diag",
        plot_dir=srpb_fuzzy_plot_dir + f"/extracted-fc-matrices/",
        metadata_dir=srpb_fuzzy_metadata_dir + f"/extracted-fc-matrices/",
        show_legend=True,
        network_assignments=network_assignments,
    )
...
def _():
    mo.md(r"""
    And we want to plot the consensus for the other metrics as well
    """)
    return


@app.function
def compute_noise_metrics_consensus(
    fuzzy_metrics_results_list: list,
    fuzzy_extracted_harmonized_fc_matrices_df_list: list,
    coords_mni,
    fuzzy_plot_dir: str,
    fuzzy_metadata_dir: str,
    network_assignments,
    target_metric_list: list[str],
):
    consensus_results_list = []

    for metric, values in fuzzy_metrics_results_list[0].items():
        if metric in target_metric_list:
            continue
        ...
        consensus_results = plot_pcs_consensus(
            ...
            fc_matrices_df_list=fuzzy_extracted_harmonized_fc_matrices_df_list,
            node_coords=coords_mni,
            pcs_to_plot=[best_pc],
            metric_to_plot=metric,
            plot_dir=fuzzy_plot_dir + "/consensus-thresholds/",
            metadata_dir=fuzzy_metadata_dir + "/consensus-thresholds/",
            consensus_thresholds=[1.0, 0.75, 0.50, 0.25],
            show_legend=True,
            network_assignments=network_assignments,
        )

        consensus_results_list.append(consensus_results)

    return consensus_results_list
...
def _(
    ...
    srpb_noise_metrics_consensus_results_list = compute_noise_metrics_consensus(
        srpb_fuzzy_metrics_results_list,
        srpb_fuzzy_extracted_harmonized_fc_matrices_df_list,
        coords_mni,
        srpb_fuzzy_plot_dir,
    ...
    return (srpb_noise_metrics_consensus_results_list,)


@app.cell
def _(
    srpb_fuzzy_metrics_results_list,
    srpb_noise_metrics_consensus_results_list,
):
    srpb_noise_metrics_ui_elements = []

    _processed_metrics = [
        _metric
        for _metric in srpb_fuzzy_metrics_results_list[0].keys()
        if _metric not in ["diag", "bdi"]
    ]

    for _metric_name, _consensus_result in zip(
        _processed_metrics, srpb_noise_metrics_consensus_results_list
    ):
        srpb_noise_metrics_ui_elements.append(mo.md(f"## {_metric_name}"))

        srpb_noise_metrics_ui_elements.append(_consensus_result["ui"])

    srpb_noise_metrics_consensus_vstack = mo.vstack(
        srpb_noise_metrics_ui_elements, gap=3
    )

    srpb_noise_metrics_consensus_vstack
    return
...
def _():
    mo.md(r"""
    ## Extraction and harmonization of the regular FC matrices of the BMB dataset
    """)
    return
...
@app.cell
def _(bmb_harmonized_fc_matrices_output_dir, regular_run_name):
    bmb_harmonized_regular_fc_matrices_output_dir = (
        bmb_harmonized_fc_matrices_output_dir / regular_run_name
    )
    return (bmb_harmonized_regular_fc_matrices_output_dir,)


@app.cell
def _(bmb_fc_matrices_bias_output_dir, regular_run_name):
    bmb_regular_fc_matrices_bias_output_dir = (
        bmb_fc_matrices_bias_output_dir / regular_run_name
    )
    return (bmb_regular_fc_matrices_bias_output_dir,)
...
def _(
    ...
    bmb_bias_dict = estimate_bias(
        output_dir_path=bmb_regular_fc_matrices_bias_output_dir,
        rs_connectivity=np.stack(
            bmb_extracted_fc_matrices_df["fc_matrix"].to_numpy(), axis=1
        ),
        subjects_to_remove_df=bmb_missing_df.select("participants_id", "session"),
        dataset="BMB",
    )
    return (bmb_bias_dict,)
...
def _(
    ...
    bmb_extracted_harmonized_fc_matrices = harmonize_connectivity(
        bias_dictionnary=bmb_bias_dict,
        connectivity=np.stack(bmb_extracted_fc_matrices_df["fc_matrix"], axis=0),
        subjects_metadata_dataframe=bmb_extracted_fc_matrices_df,
        dataset="BMB",
        output_path=bmb_harmonized_regular_fc_matrices_output_dir,
    )
    return (bmb_extracted_harmonized_fc_matrices,)


@app.cell
def _(bmb_extracted_fc_matrices_df, bmb_extracted_harmonized_fc_matrices):
    bmb_extracted_harmonized_fc_matrices_df = (
        bmb_extracted_fc_matrices_df.with_columns(
            pl.Series(
                name="harmonized_fc_matrix",
                values=bmb_extracted_harmonized_fc_matrices,
            )
        ).select(
            "participants_id",
            "session",
    ...
    return (bmb_extracted_harmonized_fc_matrices_df,)
...
@app.cell
def _(bmb_extracted_harmonized_fc_matrices_df):
    bmb_extracted_harmonized_fc_matrices_hc_mdd_df = (
        bmb_extracted_harmonized_fc_matrices_df.filter(
            pl.col("diag").is_in([0, 2])
        )
    )
    return (bmb_extracted_harmonized_fc_matrices_hc_mdd_df,)
...
def _(
    ...
    bmb_extracted_metrics_dict = calculate_metrics(
        df=bmb_filter(bmb_extracted_harmonized_fc_matrices_hc_mdd_df),
        metric_dict=metric_dict,
        alpha_threshold=0.05,
        n_pcs=5,
        plot_dir=bmb_fuzzy_plot_dir + f"/extracted-fc-matrices/",
        ttest_dir=bmb_fuzzy_ttest_dir + f"/extracted-fc-matrices/",
        cache_dir=bmb_fuzzy_cache_dir + f"/extracted-fc-matrices/",
    )

    bmb_extracted_metrics_results = bmb_extracted_metrics_dict["results"]
    bmb_extracted_metrics_selected_pcs = bmb_extracted_metrics_dict["selected_pcs"]
    bmb_extracted_metrics_ui = bmb_extracted_metrics_dict["ui"]
...
def _():
    mo.md(r"""
    We also want to compute the consensus for the different noise metrics
    """)
    return
...
def _(
    ...
    bmb_noise_metrics_consensus_results_list = compute_noise_metrics_consensus(
        bmb_fuzzy_metrics_results_list,
        bmb_fuzzy_extracted_harmonized_fc_matrices_df_list,
        coords_mni,
        bmb_fuzzy_plot_dir,
    ...
    return (bmb_noise_metrics_consensus_results_list,)


@app.cell
def _(
    bmb_fuzzy_metrics_results_list,
    bmb_noise_metrics_consensus_results_list,
):
    bmb_noise_metrics_ui_elements = []

    _processed_metrics = [
        _metric
        for _metric in bmb_fuzzy_metrics_results_list[0].keys()
        if _metric not in ["diag", "bdi"]
    ]

    for _metric_name, _consensus_result in zip(
        _processed_metrics, bmb_noise_metrics_consensus_results_list
    ):
        bmb_noise_metrics_ui_elements.append(mo.md(f"## {_metric_name}"))

        bmb_noise_metrics_ui_elements.append(_consensus_result["ui"])

    bmb_noise_metrics_consensus_vstack = mo.vstack(
        bmb_noise_metrics_ui_elements, gap=3
    )

    bmb_noise_metrics_consensus_vstack
    return
