...
def _(
    ...
    def plot_publication_ready(
        srbp_results: dict,
        harmonized_srbp_fc_matrices_df: pl.DataFrame,
        ...
        # 1. Extract Data
        target_col = harmonized_srbp_fc_matrices_df["diag"]
        mask = (
            (target_col != -1000)
            & (target_col.is_not_null())
            & ((target_col == 0) | (target_col == 2))
        )
        filtered_df = harmonized_srbp_fc_matrices_df.filter(mask)
    ...
    fig, adj = plot_publication_ready(
        srbp_results=srbp_results,
        harmonized_srbp_fc_matrices_df=harmonized_srbp_fc_matrices_df,
